...
def send_newsletter_via_brevo(title, html_content, slug, excerpt):
    ...
    # Build email HTML using concatenation, NOT an f-string, for the footer
    # section. The Brevo unsubscribe tag {{{unsubscribe}}} contains literal
    # curly braces which collide with f-string interpolation syntax and
    # require error-prone escaping (6 braces in source = 3 in output).
    # Using plain string concatenation means the tag is written exactly as
    # Brevo needs to see it, with zero escaping, zero ambiguity.
    ...
    email_html = (
        ...
        '    <div style="color:#7A9BB5;font-size:11px;letter-spacing:1.5px;margin-top:6px">' + date_str + '</div>'
        '  </div>'
        ...
        '    You\'re receiving this because you subscribed to the MoneyTailors Daily Brief.'
        ' &nbsp;&middot;&nbsp; '
        '<a href="' + SITE_URL + '" style="color:#888">MoneyTailors.com</a><br><br>'
    ...
    # Pre-send: force ALL unsubscribe hrefs to the correct Brevo tag.
    import re as _re
    email_html = _re.sub(
        r'href="[^"]*unsubscribe[^"]*"',
        'href="{{{unsubscribe}}}"',
    ...
    if '{{{unsubscribe}}}' not in email_html:
        print('WARNING: No unsubscribe tag present in email HTML.')
